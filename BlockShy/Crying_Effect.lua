...
function main()
  ...
  for i = 1, #selectedNotes do
    ...
    if step < 1000000 then step = math.floor(dur / 4) end -- 简单处理
    ...
    if vibDepth > 2.0 then vibDepth = 2.0 end
    ...
    for t = 0, 4 do
        ...
        if tensValue > 1.0 then tensValue = 1.0 end
        if tensValue < -1.0 then tensValue = -1.0 end
