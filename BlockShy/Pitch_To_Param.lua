...
function main()
  ...
  local paramLabels = { "Tension (张力)", "Breathiness (气声)", "Gender (性别)", "Tone Shift (音色)", "Loudness (响度)" }

  -- 计算平均音高供默认值使用
  local totalPitch = 0
  for _, n in ipairs(selectedNotes) do totalPitch = totalPitch + n:getPitch() end
  ...
  local inputForm = {
    ...
    widgets = {
      ...
      },
      {
        ...
        label = "参考中心音高",
        format = "%1.0f",
        minValue = 36, maxValue = 96, interval = 1,
        default = avgPitch
      },
      {
        name = "strength",
        ...
        label = "映射强度",
        format = "%1.2f",
        minValue = 0.01, maxValue = 1.0, interval = 0.01,
        default = 0.05
      },
      {
        name = "direction",
  ...
  if result.status == "Cancel" then return end
  ...
  for _, note in ipairs(selectedNotes) do
      ...
      local function calcVal(t)
          ...
          if isInverted then val = -val end
