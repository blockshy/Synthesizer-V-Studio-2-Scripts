...
- SV1 可访问的音符与参数曲线；Studio 2 音高控制点/曲线会自动跳过

## 用法

1. 在 Synthesizer V Studio 中选中要处理的轨道或音符组。
2. 从脚本菜单运行 `BPM Rescaler (SV1)`，在弹窗中填写参数。
...
6. 按需勾选是否缩放参数曲线；Studio 2 音高控制在 SV1 中会被跳过。
7. 点击 `OK` 执行。
...
- 如需 SV2.1.2+ 侧边栏版本，请使用 `BlockShy/BPM_Rescaler_SV2/BPM_Rescaler_SV2.lua`。
- 可缩放音符和参数曲线，但会自动跳过 Studio 2 音高控制对象。
